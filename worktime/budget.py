...
# TODO: упаковать в exe
# TODO: запросить расчетный период и имена файлов бюджета и лимитов

# Расчетный период в месяцах
PERIOD = input("За какое количество месяцев (включая декабрь) рассчитываем: ")
...
# Заменить форматирование столбца с номерами бегунков на "текстовый" TODO: делать автоматически
...
if (df_error['Статья'].count() > 0):
    print("ОШИБКА!!! Заполните поле ДОГОВОР у следующих бегунков!")
    print(df_error)
del df_error

#Если есть незаполненные договоры, то вывести предупреждение
df_error_dollars = df[df['Валюта'].str.contains("Доллары")]
df_error_euro = df[df['Валюта'].str.contains("Евро")]
if df_error_dollars['Статья'].count() > 0  or df_error_euro['Статья'].count() > 0:
    print("ОШИБКА!!! Переведите валюту следующих бегунков в рубли")
    print(df_error_dollars)
    print(df_error_euro)
del df_error_dollars, df_error_euro

# Приводим все суммы к виду "Без НДС"
...
ptk_limits.columns = ['Статья','Лимит (Без НДС)']
...
ptk['Лимит (Без НДС)'] = ptk['Лимит (Без НДС)'].fillna(value=0)

#Если есть незаполненные договоры, то вывести предупреждение
df_error = ptk[ptk['Лимит (Без НДС)'] == 0]
if (df_error['Лимит (Без НДС)'].count() > 0):
    print("Внимание!!! Присутствуют расходы по статьям с нулевыми лимитами!")
    print(df_error)
del df_error

# Расчитываем остатки по статьям
ptk['Остаток'] = ptk[['Без НДС', 'Лимит (Без НДС)']].apply(lambda x: x[1] - x[0], axis=1)
ptk['Равномерность расхода'] = ptk[['Без НДС', 'Лимит (Без НДС)']].apply(lambda x: x[1]/12*PERIOD - x[0], axis=1)
...
#Если есть незаполненные договоры, то вывести предупреждение
df_error = ptd[ptd['Лимит'] == 0]
if (df_error['Лимит'].count() > 0):
    print("Внимание!!! Присутствуют расходы по договорам с нулевыми лимитами!")
    print(df_error)
del df_error
